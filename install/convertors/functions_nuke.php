...
/**
* Helper functions for NUKE phpBB 2.0.x to phpBB 3.0.x conversion
*/
...
function phpbb_insert_forums()
{
	...
	$sql = 'SELECT MAX(forum_id) AS max_forum_id
		FROM ' . $convert->src_table_prefix . 'bbforums';
	...
	$sql = "SELECT config_value
		FROM {$convert->src_table_prefix}bbconfig
	...
	$sql = 'SELECT cat_id, cat_title
		FROM ' . $convert->src_table_prefix . 'bbcategories
		ORDER BY cat_order';
...
/**
* UN hack for converting text date format to timestamp
* 
* @param string $user_regdate Registration date of user in text format
* @return int Text format transformed to timestamp
*/
function phpbb_regdate_timestamp($user_regdate)
{
...
/**
* Correct the smiley and avatar path - strip module path
* 
* @param string $path The original path which shouldn't include the board root path
* @return string Correct path
*/
function phpbb_strip_module_path($path)
{
	if (strpos($path, 'modules/Forums/') !== false) 
  {
    $path = str_replace('modules/Forums/', '', $path);
  }
	return $path;
}
